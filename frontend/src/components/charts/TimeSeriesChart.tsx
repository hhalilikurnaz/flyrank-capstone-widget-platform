  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full" role="img" aria-label="Submissions over time">
        ...
        <text x={4} y={PAD_TOP + 4} fontSize={10} className="fill-slate-500 dark:fill-slate-400">
          {maxCount}
        </text>
        <text x={4} y={PAD_TOP + plotHeight} fontSize={10} className="fill-slate-500 dark:fill-slate-400">
          0
        </text>
      ...
      {hovered && (
        <div
          className="pointer-events-none absolute top-2 -translate-x-1/2 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs shadow-sm dark:border-slate-700 dark:bg-slate-800"
          style={{ left: `${(hovered.x / WIDTH) * 100}%` }}
        >
          <p className="font-medium text-slate-900 dark:text-slate-100">{hovered.count} submissions</p>
          <p className="text-slate-500 dark:text-slate-400">{formatShortDate(hovered.date)}</p>
        </div>
      )}
    </div>
  );
